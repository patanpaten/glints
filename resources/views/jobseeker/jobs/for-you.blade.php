    ...
    <div class="container mx-auto px-4 py-6">
        <div class="max-w-7xl mx-auto">
            <div class="flex gap-6">
                ...
                <div class="w-1/2">
                    <div class="Box__StyledBox-sc-pr7b7l-0 kgsHid Flex__StyledFlex-sc-11ryoct-0 evNJxH ForYouTabsc__QRBannerContainer-sc-c9mqd3-2 cfVYNj">
                        <div id="fyp-tab-qr-banner" class="MobileAppDownloadQRBannersc__QRSectionContainer-sc-4pu5ro-0 ilPNCt flex items-center space-x-4">
                        ...
                        <div onclick="showJobDetail({{ $job->id }})"
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <!-- Job Title -->
                                    <div class="mb-2">
                                        <h2 class="text-lg font-semibold text-gray-900 leading-tight group-hover:text-blue-600 transition-colors line-clamp-2">
                                            {{ $job->title }}
                                        </h2>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        @if($job->salary_min && $job->salary_max && !$job->hide_salary)
                                            <span class="text-base font-bold text-green-600">
                                                Rp {{ number_format($job->salary_min/1000000, 1, ',', '.') }}-{{ number_format($job->salary_max/1000000, 1, ',', '.') }} jt
                                            </span>
                                        @else
                                            <span class="text-sm text-gray-500">Gaji akan dibahas saat interview</span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex flex-wrap gap-2 mb-3">
                                        ...
                                        <span class="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">
                                            {{ ucfirst(str_replace('_', ' ', strtolower($job->employment_type))) }}
                                        </span>
                                        ...
                                        @if($job->work_system)
                                            <span class="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full font-medium">
                                                {{ ucfirst(str_replace('_', ' ', strtolower($job->work_system))) }}
                                            </span>
                                        @endif
                                        ...
                                        @if($job->education_level)
                                            <span class="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full">
                                                {{ $job->education_level == 'BACHELOR_DEGREE' ? 'S1' : ($job->education_level == 'HIGH_SCHOOL' ? 'SMA/SMK' : ucfirst(str_replace('_', ' ', strtolower($job->education_level)))) }}
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <!-- HOT Badge -->
                                            @if($job->created_at->diffInHours() < 24)
                                            ...
                                            @if($job->is_active)
                                                <span class="px-2 py-1 bg-green-600 text-white text-xs rounded-full font-medium">
                                            ...
                                            <span class="text-xs text-gray-500">
                                                @if($job->created_at->diffInHours() < 1)
                                                    {{ $job->created_at->diffInMinutes() }} menit yang lalu
                                                @elseif($job->created_at->diffInHours() < 24)
                                                    {{ $job->created_at->diffInHours() }} jam yang lalu
                                                @elseif($job->created_at->diffInDays() < 7)
                                                    {{ $job->created_at->diffInDays() }} hari yang lalu
                                                @else
